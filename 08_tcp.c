...
/*
 * Prepares a TCP segment for transmission.
 *
 * Parameters:
 *     - s: The socket descriptor.
 *     - flags: The TCP flags to be set in the segment.
 *     - payload: Pointer to the payload data.
 *     - payloadlen: Length of the payload data.
 *     - options: Pointer to the TCP options.
 *     - optlen: Length of the TCP options.
 *
 * Returns:
...
int prepare_tcp(int s, unsigned char flags, unsigned char *payload, int payloadlen, unsigned char *options, int optlen) {
    ...
    txcb->txtime = -MAXTIMEOUT; // Set transmission timeout
    txcb->payloadlen = payloadlen; // Set payload length
    txcb->totlen = payloadlen + 20 + optlen; // Calculate total segment length
    txcb->retry = 0; // Initialize retry count
    tcp = txcb->segment = (struct tcp_segment *) malloc(sizeof(struct tcp_segment)); // Allocate memory for TCP segment

    tcp->s_port = fdinfo[s].l_port; // Set source port
    tcp->d_port = t->r_port; // Set destination port
    if (t->r_port == 0)
        ; // printf("Illegal Packet...\n"); // Handle illegal packet case
    tcp->seq = htonl(t->seq_offs + t->sequence); // Set sequence number
    tcp->d_offs_res = (5 + optlen / 4) << 4; // Set data offset and reserved bits
    tcp->flags = flags & 0x3F; // Set TCP flags
    tcp->urgp = 0; // Set urgent pointer
    if (options != NULL)
        memcpy(tcp->payload, options, optlen); // Copy TCP options to payload
    if (payload != NULL)
        memcpy(tcp->payload + (optlen / 4) * 4, payload, payloadlen); // Copy payload data
    txcb->next = NULL; // Set next transmission control buffer to NULL
    if (t->txfirst == NULL) {
        t->txlast = t->txfirst = txcb; // Set the first transmission control buffer
    } else {
        t->txlast->next = txcb; // Append transmission control buffer to the end
        t->txlast = t->txlast->next; // Update the last transmission control buffer
    }
    printf("%.7ld: Packet seq inserted %d:%d\n", rtclock(0), t->sequence, t->sequence + payloadlen); // Print packet sequence information
    t->sequence += payloadlen; // Update the sequence number
    // DEFERRED FIELDS
    // tcp->ack;
    // tcp->window;
    // tcp->checksum=0;
...
/*
 * Resolves the MAC address for a given destination IP address.
 *
 * Parameters:
 *     - destip: The destination IP address.
 *     - destmac: Pointer to store the resolved MAC address.
 *
 * Returns:
 *     0 if the MAC address is resolved successfully, -1 otherwise.
 */
int resolve_mac(unsigned int destip, unsigned char *destmac) {
    int len, n, i;
    clock_t start;
    ...
    if (arpcache[i].key) { // If found in cache, return
    ...
    eth->type = htons(0x0806); // Set Ethernet type as ARP

    arp->htype = htons(1); // Set hardware type as Ethernet
    arp->ptype = htons(0x0800); // Set protocol type as IPv4
    arp->hlen = 6; // Set hardware address length
    arp->plen = 4; // Set protocol address length
    arp->op = htons(1); // Set ARP operation as request

    // Set source MAC address in ARP packet
    for (i = 0; i < 6; i++)
        arp->srcmac[i] = mymac[i];

    // Set source IP address in ARP packet
    for (i = 0; i < 4; i++)
        arp->srcip[i] = myip[i];

    // Set destination MAC address in ARP packet as unknown
    for (i = 0; i < 6; i++)
        arp->dstmac[i] = 0;

    // Set destination IP address in ARP packet
    for (i = 0; i < 4; i++)
    ...
    while (pause()) { // Wake up only upon signals
        // Check if the MAC address is resolved and present in cache
        for (i = 0; (i < MAX_ARP) && (arpcache[i].key != 0); i++) {
            if (!memcmp(&arpcache[i].key, &destip, 4))
                break;
        }
...
/*
 * Updates the TCP header fields in the transmission control buffer.
 *
 * Parameters:
 *     - s: The socket descriptor.
 *     - txctrl: Pointer to the transmission control buffer.
 *
 * Returns:
 *     The function does not return a value.
 */
void update_tcp_header(int s, struct txcontrolbuf *txctrl) {
    ...
    txctrl->segment->checksum = htons(0); // Set checksum field to 0 temporarily

    txctrl->segment->ack = htonl(tcb->ack_offs + tcb->cumulativeack); // Set acknowledgment number
    txctrl->segment->window = htons(tcb->adwin); // Set window size
...
/*
 * Creates a socket and initializes the socket information.
 *
 * Parameters:
 *     - family: The socket family (AF_INET for IPv4).
 *     - type: The socket type (SOCK_STREAM for TCP).
 *     - proto: The protocol (0 for default).
 *
 * Returns:
 *     The socket descriptor if successful, -1 otherwise.
 */
int mysocket(int family, int type, int proto) {
    ...
    if ((family == AF_INET) && (type == SOCK_STREAM) && (proto == 0)) {
        ...
        if (i == MAX_FD) {
            myerrno = ENFILE; // Set error number
            return -1;
        } else {
            bzero(fdinfo + i, sizeof(struct socket_info)); // Initialize socket info
            fdinfo[i].st = TCP_UNBOUND; // Set socket state
            myerrno = 0; // Reset error number
            return i; // Return socket descriptor
        }
    } else {
        myerrno = EINVAL; // Set error number
        return -1;
    }
}
...
/*
 * Checks if a given port is in use by any socket.
 *
 * Parameters:
 *     - port: The port number to check.
 *
 * Returns:
 *     1 if the port is in use, 0 otherwise.
 */
 ...
 *
 * Returns:
 *     The free port number if available, 0 otherwise.
 */
unsigned short int get_free_port() {
    unsigned short p;
    for (p = last_port; p < MAX_PORT && port_in_use(p); p++);
    if (p < MAX_PORT) {
        return last_port = p; // Return available port
    }
    for (p = MIN_PORT; p < last_port && port_in_use(p); p++);
    if (p < last_port) {
        return last_port = p; // Return available port
    }
    return 0; // No free port available
}
 ...
 * Parameters:
 *     - s: The socket descriptor.
 *     - addr: Pointer to the sockaddr structure containing the address information.
...
/*
 * Implements the Finite State Machine (FSM) for TCP.
 *
 * Parameters:
 *     - s: The socket descriptor.
 *     - event: The event triggering the FSM.
 *     - ip: Pointer to the received IP datagram (NULL if not applicable).
 *
 * Returns:
 *     The function does not return a value.
 */
    ...
    switch (tcb->st) {
        case TCP_CLOSED:
            if (event == APP_ACTIVE_OPEN) {
                tcb->rxbuffer = (unsigned char *)malloc(RXBUFSIZE); // Allocate receive buffer
                tcb->txfree = TXBUFSIZE; // Set available transmit buffer size
                tcb->seq_offs = rand(); // Generate random sequence offset
                tcb->ack_offs = 0; // Set acknowledgment offset
                tcb->stream_end = 0xFFFFFFFF; // Set maximum file size
                tcb->mss = TCP_MSS; // Set maximum segment size
                tcb->sequence = 0; // Initialize sequence number
                tcb->rx_win_start = 0; // Initialize receive window start
                tcb->cumulativeack = 0; // Initialize cumulative acknowledgment
                tcb->timeout = INIT_TIMEOUT; // Set initial timeout
                tcb->adwin = RXBUFSIZE; // Set advertised receive window size
                tcb->radwin = RXBUFSIZE; // Set receive window size for congestion control

#ifdef CONGCTRL
                tcb->ssthreshold = INIT_THRESH * TCP_MSS; // Set slow start threshold
                tcb->cgwin = INIT_CGWIN * TCP_MSS; // Set congestion window size
                tcb->timeout = INIT_TIMEOUT; // Set timeout
                tcb->rtt_e = 0; // Initialize RTT estimation
                tcb->Drtt_e = 0; // Initialize RTT deviation estimation
                tcb->cong_st = SLOW_START; // Set congestion state
#endif

                prepare_tcp(s, SYN, NULL, 0, mssopt, sizeof(mssopt)); // Prepare TCP segment with SYN flag
                tcb->st = SYN_SENT; // Transition to SYN_SENT state
            }
            break;
        ...
        case SYN_SENT:
            if (event == PKT_RCV) {
                if ((tcp->flags & SYN) && (tcp->flags & ACK) && (htonl(tcp->ack) == tcb->seq_offs + 1)) {
                    tcb->seq_offs++; // Increment sequence offset
                    tcb->ack_offs = htonl(tcp->seq) + 1; // Set acknowledgment offset
                    free(tcb->txfirst->segment); // Free first transmit segment
                    free(tcb->txfirst); // Free first transmit control buffer
                    tcb->txfirst = tcb->txlast = NULL; // Reset transmit control buffer pointers
                    prepare_tcp(s, ACK, NULL, 0, NULL, 0); // Prepare TCP segment with ACK flag
                    tcb->st = ESTABLISHED; // Transition to ESTABLISHED state
                }
            }
            break;
        ...
        case ESTABLISHED:
            if (event == PKT_RCV && (tcp->flags & FIN))
                tcb->st = CLOSE_WAIT; // Transition to CLOSE_WAIT state
            else if (event == APP_CLOSE) {
                prepare_tcp(s, FIN | ACK, NULL, 0, NULL, 0); // Prepare TCP segment with FIN and ACK flags
                tcb->st = FIN_WAIT_1; // Transition to FIN_WAIT_1 state
            }
            break;
        ...
        case CLOSE_WAIT:
            if (event == APP_CLOSE) {
                prepare_tcp(s, FIN | ACK, NULL, 0, NULL, 0); // Prepare TCP segment with FIN and ACK flags
                tcb->st = LAST_ACK; // Transition to LAST_ACK state
            }
            break;
        ...
        case LAST_ACK:
            if ((event == PKT_RCV) && (tcp->flags & ACK)) {
                if (htonl(tcp->ack) == (tcb->seq_offs + tcb->sequence + 1)) {
                    tcb->st = TCP_CLOSED; // Transition to TCP_CLOSED state
                    tcb->txfirst = tcb->txlast = NULL; // Reset transmit control buffer pointers
                }
            }
            break;
        ...
        case LISTEN:
            if ((event == PKT_RCV) && ((tcp->flags) & SYN)) {
                ...
                tcb->txfree = TXBUFSIZE; // Set available transmit buffer size
                tcb->ack_offs = htonl(tcp->seq) + 1; // Set acknowledgment offset
                tcb->r_port = tcp->s_port; // Set remote port
                tcb->r_addr = ip->srcaddr; // Set remote address
                tcb->rx_win_start = 0; // Initialize receive window start
                tcb->cumulativeack = 0; // Initialize cumulative acknowledgment
                tcb->adwin = RXBUFSIZE; // Set advertised receive window size
                tcb->radwin = RXBUFSIZE; // Set receive window size for congestion control
                ...
                prepare_tcp(s, SYN | ACK, NULL, 0, mssopt, sizeof(mssopt)); // Prepare TCP segment with SYN and ACK flags
                tcb->st = SYN_RECEIVED; // Transition to SYN_RECEIVED state
            }
            break;
            ...
            if (((event == PKT_RCV) && ((tcp->flags) & ACK)) && !((tcp->flags) & SYN)) {
                if (htonl(tcp->ack) == tcb->seq_offs + 1) {
                    ...
                    if (fdinfo[s].tcblist[i].st != FREE)
                        prepare_tcp(s, RST, NULL, 0, NULL, 0); // Prepare TCP segment with RST flag
                    else {
                        fdinfo[s].tcblist[i] = *tcb; // Copy TCB to listening socket's TCB list
                        fdinfo[s].tcblist[i].st = ESTABLISHED; // Set state to ESTABLISHED
                    }
                    ...
                    tcb->st = LISTEN; // Transition to LISTEN state
                }
            }
            break;
        ...
        case FIN_WAIT_1:
            if ((event == PKT_RCV) && ((tcp->flags) & FIN)) {
                tcb->st = CLOSING; // Transition to CLOSING state
                // Acknowledgment will be sent as cumulative.
            } else if ((event == PKT_RCV) && ((tcp->flags) & ACK)) {
                if (htonl(tcp->ack) == tcb->seq_offs + tcb->sequence + 1)
                    tcb->st = FIN_WAIT_2; // Transition to FIN_WAIT_2 state
            }
            break;
        ...
        case FIN_WAIT_2:
            if ((event == PKT_RCV) && ((tcp->flags) & FIN)) {
                ...
                tcb->st = TIME_WAIT; // Transition to TIME_WAIT state
        ...
        case TIME_WAIT:
            if (event == TIMEOUT) {
                ...
                free(tcb->rxbuffer); // Free receive buffer
                free(fdinfo[s].tcb); // Free TCB memory
                bzero(fdinfo + s, sizeof(struct socket_info)); // Clear socket info
                fdinfo[s].st = FREE; // Set socket state to FREE
            }
            break;
    }
...
/*
 * Initiates a TCP connection to a remote address.
 *
 * Parameters:
 *     - s: The socket descriptor.
 *     - addr: Pointer to the sockaddr structure containing the remote address.
 *     - addrlen: Length of the sockaddr structure.
 *
 * Returns:
 *     0 if the connection is established successfully, -1 otherwise.
 */
 ...
 *     - maxlen: Maximum length of data to write.
 *
 * Returns:
 *     The total number of bytes written, or -1 if an error occurs.
 */
int mywrite(int s, unsigned char *buffer, int maxlen) {
    ...
    if (fdinfo[s].st != TCB_CREATED || fdinfo[s].tcb->st != ESTABLISHED) {
        myerrno = EINVAL; // Set error number to EINVAL
        return -1;
    }
    ...
    do {
        ...
        if ((actual_len != 0) || (fdinfo[s].tcb->st == TCP_CLOSED))
            break;
    } while (pause()); // Wait until data can be sent or the connection is closed

    for (j = 0; j < actual_len; j += fdinfo[s].tcb->mss) {
        len = MIN(fdinfo[s].tcb->mss, actual_len - j);
        if (-1 == sigprocmask(SIG_BLOCK, &mymask, NULL)) { // Block the signal
            perror("sigprocmask");
            return -1;
        }
        prepare_tcp(s, ACK, buffer + j, len, NULL, 0); // Prepare TCP segment with ACK flag
        if (-1 == sigprocmask(SIG_UNBLOCK, &mymask, NULL)) { // Unblock the signal
            perror("sigprocmask");
            return -1;
        }
        fdinfo[s].tcb->txfree -= len; // Update the available space in the transmit buffer
        totlen += len; // Update the total number of bytes written
    }
    ...
    if (actual_len == 0) {
        while (pause()) {
            ...
            if ((fdinfo[s].tcb->st == CLOSE_WAIT) && (fdinfo[s].tcb->unack == NULL)) {
                return 0; // FIN received and acknowledged
            }
        }
    }

    for (j = 0; j < actual_len; j++) {
        buffer[j] = fdinfo[s].tcb->rxbuffer[(fdinfo[s].tcb->rx_win_start + j) % RXBUFSIZE]; // Read data from the receive buffer
    }

    fdinfo[s].tcb->rx_win_start += j; // Update the receive window start position
...
int myclose(int s) {
    if ((fdinfo[s].st == TCP_CLOSED) || (fdinfo[s].st == TCP_UNBOUND)) {
        myerrno = EBADF; // Set error number to EBADF
        return -1;
    }

    fsm(s, APP_CLOSE, NULL); // Perform the appropriate FSM transition

    return 0;
}

/*
 * Timer handler for TCP sockets.
 *
 * Parameters:
 *     - number: Timer number.
 *
...
void mytimer(int number) {
    ...
    fl++; // Increment the global variable
    ...
    for (i = 0; i < MAX_FD; i++) {
        if (fdinfo[i].st == TCB_CREATED) {
            ...
            if ((tcb->fsm_timer != 0) && (tcb->fsm_timer < tick)) { // Check if FSM timer has expired
                fsm(i, TIMEOUT, NULL); // Trigger FSM with TIMEOUT event
                continue;
            }
            ...
            for (tot = 0, txcb = tcb->txfirst; txcb != NULL /*&& (tot<tcb->radwin)*/; txcb = txcb->next) {
                ...
                update_tcp_header(i, txcb); // Update TCP header
                send_ip((unsigned char *)txcb->segment, (unsigned char *)&fdinfo[i].tcb->r_addr, txcb->totlen, TCP_PROTO); // Send the TCP segment
                ...
                if ((txcb->retry > 1) && (tcb->st >= ESTABLISHED) && !isfasttransmit)
                    congctrl_fsm(tcb, TIMEOUT, NULL, 0); // Congestion control FSM
    ...
    fl--; // Decrement the global variable
...
/*
 * Print the contents of the receive queue.
 *
 * Parameters:
 *     - n: Pointer to the head of the receive queue.
 *
 * Returns:
 *     The function does not return a value.
 */
...
/*
 * I/O processing function for handling network packets.
 *
 * Parameters:
 *     - number: Timer number (unused).
 *
 * Returns:
 *     The function does not return a value.
 */
            ...
            if (eth->type == htons(0x0806)) {
                ...
                if (ip->proto == TCP_PROTO) {
                    ...
                    if (i == MAX_FD) { // If not found connected TCB, second choice: listening socket
                        for (i = 0; i < MAX_FD; i++) { // For each TCB, find listening sockets
...
/*
 * Listen function to mark a socket as a passive opener and create a pending connection backlog.
 *
 * Parameters:
 *     - s: Socket descriptor.
 *     - bl: Backlog length.
...
/*
 * Accept function to accept a pending connection and create a new connected socket.
 *
 * Parameters:
 *     - s: Socket descriptor.
 *     - addr: Pointer to sockaddr structure to store the remote address.
 *     - len: Pointer to the length of the sockaddr structure.
 *
 * Returns:
 *     New socket descriptor on success, -1 on failure.
 */
int myaccept(int s, struct sockaddr *addr, int *len) {
    ...
    if (addr->sa_family == AF_INET) {
        ...
        *len = sizeof(struct sockaddr_in);
        ...
        if (fdinfo[s].tcb->st != LISTEN) {  // Check if socket is in LISTEN state
            myerrno = EBADF;
            return -1;  // Invalid socket state
        }

        if (fdinfo[s].tcblist == NULL) {
            myerrno = EBADF;
            return -1;  // No pending connections in backlog
        }

        do {
            for (i = 0; i < fdinfo[s].bl; i++) {
                if (fdinfo[s].tcblist[i].st == ESTABLISHED) {  // Found a pending connection in backlog
                    for (j = 3; j < MAX_FD && fdinfo[j].st != FREE; j++);  // Searching for free descriptor

                    if (j == MAX_FD) {
                        myerrno = ENFILE;
                        return -1;  // No free descriptor available
                    } else {
                        fdinfo[j] = fdinfo[s];  // Copy the listening socket's information
                        fdinfo[j].tcb = (struct tcpctrlblk *)malloc(sizeof(struct tcpctrlblk));
                        memcpy(fdinfo[j].tcb, &fdinfo[s].tcblist[i], sizeof(struct tcpctrlblk));  // Copy tcb from pending connection in backlog
                        a->sin_port = fdinfo[j].tcb->r_port;  // Report the remote port
                        a->sin_addr.s_addr = fdinfo[j].tcb->r_addr;  // Report the remote IP address
                        fdinfo[j].bl = 0;  // Twin socket has no backlog queue
                        fdinfo[s].tcblist[i].st = FREE;  // Mark the pending connection as free
                        printf("%.7ld: Reset clock\n", rtclock(1));
                        prepare_tcp(j, ACK, NULL, 0, NULL, 0);  // Prepare an ACK segment
                        return j;  // Return the new socket descriptor
                    }
                }
            }
        ...
        myerrno = EINVAL;
        return -1;  // Invalid address family
    }
}
...
int main(int argc, char **argv) {
    ...
    // Set signal handlers for I/O and timer
    ...
    // Set the process ID to receive the asynchronous I/O signals
    ...
    // Enable asynchronous I/O and non-blocking mode for the socket
    ...
    // Set up the socket address structure for raw packet capturing
    sll.sll_family = AF_PACKET;
    sll.sll_ifindex = if_nametoindex("eth0");

    // Set the timer interval for periodic timer
    myt.it_interval.tv_sec = 0;
    myt.it_interval.tv_usec = TIMER_USECS;
    myt.it_value.tv_sec = 0;
    myt.it_value.tv_usec = TIMER_USECS;

    // Set up the signal mask for blocking signals
    ...
    // Set the timer for periodic timer interrupts
    ...
    printf("Port: %d, TXBUFSIZE :%d , TIMEOUT: %d MODE:%s INV.LOSSRATE:%d\n",
           atoi(argv[1]), TXBUFSIZE, INIT_TIMEOUT * TIMER_USECS / 1000,
           (argc >= 5) ? argv[4] : "SRV", INV_LOSS_RATE);
    ...
    if (argc >= 5 && !strcmp(argv[4], "CLN")) {
        ...
        while (1) {
            ...
            // Process the HTTP request and generate the response
